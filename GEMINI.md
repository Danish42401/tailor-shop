...
### [2026-05-01 11:30 AM] - Transition to 'Price on Request' Model
**User Prompt:** Remove prices from products and show "Price on Request" instead. Don't want to enter prices in Google Sheets.
**Plan:**
- Added "Price on Request" translations (EN/AR).
- Updated `data-service.ts` to handle empty price cells gracefully (default to 0 internally).
- Removed all numeric price displays from Home, Catalog, Product Detail, and Cart UI.
- Updated WhatsApp message logic to exclude prices and act as a quote inquiry.
- Verified production build compatibility for Cloudflare Edge.
**Action:** Updated `LanguageContext.tsx`, `data-service.ts`, `page.tsx`, `Catalog/page.tsx`, `Product/[id]/page.tsx`, `CartDrawer.tsx`, `OrderSummaryModal.tsx`, and `whatsapp.ts`.
**Git:** Committed and pushed to `origin main`.
**Status:** Completed & Synced.
