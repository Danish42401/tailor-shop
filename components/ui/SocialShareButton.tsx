...
export function SocialShareButton({
    ...
    const shareMessage = locale === 'ar'
        ? `👗 شاهدي هذا المنتج من ${shopName}:\n${productName}\n💰 السعر: AED ${price.toFixed(2)}\n${productUrl}`
        : `👗 Check out this product from ${shopName}:\n${productName}\n💰 Price: AED ${price.toFixed(2)}\n${productUrl}`;

    const waUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(shareMessage)}`;
    ...
    return (
        <div className={`flex items-center gap-2 ${className}`}>
            <a
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#25D366] text-white text-sm font-medium hover:bg-[#128C7E] transition-colors"
